...
struct ngx_http_upstream_s {
    ...
    // buffer成员存储接收自上游服务器发来的响应内容，由于它会被复用，所有具有下列多种意义：
    //      a)在使用process_header方法解析上游响应的包头时，buffer中将会保存完整的响应包头;
    //      b)当下面的buffering成员为１，而且此时upstream是向下游转发上游的包体时，buffer没有意义;
    //      c)当buffering标志位为０时，buffer缓冲区会被用于反复接收上游的包体，进而向下游转发;
    ...
    // 那么是在告诉upstream还没有收到完整的响应包头，此时，对于本次upstream请求来说，再次接收到上游服务器
    // 发来的TCP流时，还会调用process_hander方法处理，直到process_header函数返回非NGX_AGAIN值这一阶段才会停止
    ...
    // 在向客服端转发上游服务器的包体时才会调用．当buffering为１时，表示使用多个缓冲区以及磁盘文件
    // 来转发上游的响应包体．当Nginx与上游间的网速远大于Nginx与下游客服端间的网速时，让Nginx开辟更多的
    // 的内存甚至使用磁盘文件来缓存上游的响应包体，这是有意义的，它可以减轻上游服务器的并发压力．当buffering
